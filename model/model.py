...
class DiSANN(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, dropout, hidden_size, num_class, device):
        super(DiSANN, self).__init__()
        ...
        self.elu = nn.ELU()
        self.disan = DiSAN(d_e=embedding_dim,
                            d_h=hidden_size,
                            dropout=dropout,
                            device=self.device)

        self.fc = nn.Linear(hidden_size * 8, hidden_size)
        self.fc_out = nn.Linear(hidden_size, num_class)
    ...
    def forward(self, premise, pre_length, hypothesis, hyp_length):
        _, p_seq_len = premise.size()
        _, h_seq_len = hypothesis.size()
        p_rep_mask = self._get_rep_mask(pre_length, p_seq_len, self.device)
        h_rep_mask = self._get_rep_mask(hyp_length, h_seq_len, self.device)
        # Embedding
        pre_x = self.embedding(premise)
        hypo_x = self.embedding(hypothesis)
        # DiSAN
    ...
    def _get_rep_mask(self, lengths, sentence_len, device):
