...
class DiSA(nn.Module):
	...
	def __init__(self, d_e, d_h, dropout, direction, device):
		...
		self.fc = nn.Linear(self.d_e, self.d_h)
		init.xavier_uniform_(self.fc.weight.data)
		init.constant_(self.fc.bias.data, 0)

		self.w_1 = nn.Linear(self.d_h, self.d_h)
		self.w_2 = nn.Linear(self.d_h, self.d_h)
		...
		self.b_1 = nn.Parameter(torch.zeros(self.d_h))
		self.c = nn.Parameter(torch.Tensor([5.0]), requires_grad=False)

		self.w_f1 = nn.Linear(self.d_h, self.d_h)
		self.w_f2 = nn.Linear(self.d_h, self.d_h)
		...
		self.b_f = nn.Parameter(torch.zeros(self.d_h))
...
class DiSAN(nn.Module):
	...
	def __init__(self, d_e, d_h, dropout, device):
		super(DiSAN, self).__init__()

		self.d_e = d_e
		self.d_h = d_h

		self.fw_DiSA = DiSA(self.d_e, self.d_h, dropout, direction='fw', device=device)
		self.bw_DiSA = DiSA(self.d_e, self.d_h, dropout, direction='bw', device=device)

		self.source2token = Source2Token(self.d_h * 2, dropout)
